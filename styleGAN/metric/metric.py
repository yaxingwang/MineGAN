...
def get_fake_images_and_acts(inception, g_running, code_size, step, alpha, sample_num=5000, batch_size=16, miner=None, miner_semantic=None):
	...
	for gen_in in loader:
		...
		with torch.no_grad():
			fake_image = g_running(miner(gen_in), step=step, alpha=alpha, miner_semantic=miner_semantic)

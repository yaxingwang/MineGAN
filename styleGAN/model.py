    ...
    def forward(
        ...
        if noise is None:
            ...
            for i in range(step + 1):
                size = 4 * 2 ** i
                if i==0 and miner_semantic is not None:
                    noise_ = torch.randn(batch,  size*size, device=input[0].device)
                    miner_output = miner_semantic(noise_)
                if i<4 and miner_semantic is not None:
                    noise.append(miner_output[i].view(batch, 1, size, size))
                else:
...
class Miner_semantic(nn.Module):
    def __init__(self, code_dim=16):
        ...
        self.transform_8 = nn.Sequential(
            nn.ReLU(),
            nn.Linear(code_dim, 4*code_dim),
        )
        self.transform_16 = nn.Sequential(
            nn.ReLU(),
            nn.Linear(4*code_dim, 16*code_dim),
        )
        self.transform_32 = nn.Sequential(
            nn.ReLU(),
            nn.Linear(16*code_dim, 64*code_dim),
        )
        self.transform_64 = nn.Sequential(
            nn.ReLU(),
            nn.Linear(64*code_dim, 256*code_dim),
        )
        self.transform_128 = nn.Sequential(
            nn.ReLU(),
            nn.Linear(256*code_dim, 1024*code_dim),
        )
    ...
    def forward(self, input):
        ...
        output4 = self.transform_4(input) 
        output8 = self.transform_8(output4) 
        output16 = self.transform_16(output8) 
        output32 = self.transform_32(output16) 
        return [output4, output8, output16, output32]

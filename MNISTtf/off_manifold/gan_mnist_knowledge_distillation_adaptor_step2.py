...
FOLDER_NAME = 'home1_step2_nlen%s/ada_input_len_%d'%(NOISE_LEN,ADAPTOR_INPUT_LEN)
...
if ADAPTOR: 
    ...
    MODEL_STEP1 = 'WGAN_GP.model-%s'%str(ITERS-CHECKPOINT_STEP)
    #for i in range(1,len(SELECTING_LABEL)):
    LABEL_DIR_STEP1=''.join(np.array(BIAS_DIGIT).astype(str))
    MODEL_DIR_STEP1 = RESULT_DIR_STEP1 + '/'+ LABEL_DIR_STEP1 + '/model/%s'%MODEL_STEP1
else:
...
gen_params = lib.params_with_name('%s_Adaptor_Generator'%ID_TEACHER)
...
# For saving samples
fixed_noise_adaptor = tf.constant(np.random.normal(size=(N_SAMPLES, ADAPTOR_INPUT_LEN)).astype('float32'))
...
p_max=np.zeros((CRITIC_ITERS))
best_i=np.zeros((ITERS))
p_i=np.zeros((CRITIC_ITERS))
...
with tf.Session() as session:
    ...
    if ADAPTOR:
        ...
        print(tf.train.list_variables(MODEL_DIR_STEP1))
    ...
    print("---PARAM VALUES---")
    tvars = tf.trainable_variables() #tvars = tf.global_variables()
    tvars_vals = session.run(tvars)
    for var, val in zip(tvars, tvars_vals):
        print(var.name, np.shape(val))
        #print(var.name, val)

    #print("---ALL VARIABLES FROM TF---")
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(disc_iters):
            ...
            _disc_cost, _ , _disc_output = session.run(
                [disc_cost, disc_train_op, disc_fake],
            ...
            #    print(var.name, np.shape(val))
            #    print(var.name, val)
            output_D_FC=tvars_vals[18]
            output_D_FC_normalized=np.divide((output_D_FC-np.min(output_D_FC)),(np.max(output_D_FC)-np.min(output_D_FC)))
            p_max[i]=np.max(output_D_FC) #max of normalized would be 1
        best_i[iteration]=np.argmax(p_max)
...
#            lib.plot.plot('%s/dev disc cost'%SAMPLES_DIR, np.mean(dev_disc_costs))
        ...
        if iteration % CHECKPOINT_STEP == 0:
            ...
            for i in xrange(disc_iters): #critic_iters
                 p_i[i]=np.sum(best_i==i)/np.size(best_i)
        ...
        #    generate_image_test_iter(i,iteration)
            #print i
    pickle.dump( all_costs, open( "%s/costs.pkl"%SAMPLES_DIR, "wb" ) )
    pickle.dump( all_costs_dev, open( "%s/costs_dev.pkl"%SAMPLES_DIR, "wb" ) )

...
#note: load pretrained model from step 1, miner (Genrator_Adaptor) adapts source to target BIAS_DIGIT, update G from step 2
...
ADAPTOR_INPUT_LEN = 4
FOLDER_NAME = 'home1_step3_nlen%s/ada_input_len_%d'%(NOISE_LEN,ADAPTOR_INPUT_LEN)
...
MODEL_STEP2 = 'WGAN_GP.model-%s'%str(ITERS-CHECKPOINT_STEP)
MODEL_DIR_STEP2 = RESULT_DIR_STEP2 + '/'+ LABEL_DIR + '/model/%s'%MODEL_STEP2
...
gen_params = [p for p in lib.params_with_name('%s_Generator'%ID_TEACHER)] + [p for p in lib.params_with_name('%s_Adaptor_Generator'%ID_TEACHER)]
disc_params = lib.params_with_name('%s_Discriminator'%ID_TEACHER)

pretrained_params = [p for p in lib.params_with_name('%s_Generator'%ID_TEACHER)] +  [p for p in lib.params_with_name('%s_Discriminator'%ID_TEACHER)] + [p for p in lib.params_with_name('%s_Adaptor_Generator'%ID_TEACHER)]

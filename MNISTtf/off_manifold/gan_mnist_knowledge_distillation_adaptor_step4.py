...
#note: load pretrained model from step 1, miner (Genrator_Adaptor) adapts source to target BIAS_DIGIT, update G from step 2, distillation
...
SELECTING_LABEL = [BIAS_DIGIT, 8] # digits are 1 and 4
N_SAMPLES=20
LIMIT = 8000
L2_LAMBDA = 0
...
ADAPTOR_INPUT_LEN = 4
FOLDER_NAME = 'home1_step4_nlen%s/ada_input_len_%d'%(NOISE_LEN,ADAPTOR_INPUT_LEN)
...
RESULT_DIR_STEP3 = './result/%s/%s'%FOLDER_NAME_STEP3%str(SELECTING_LABEL[0]) 
MODEL_STEP3 = 'WGAN_GP.model-%s'%str(ITERS-CHECKPOINT_STEP)
MODEL_DIR_STEP3 = RESULT_DIR_STEP3 + '/'+ str(SELECTING_LABEL[0]) + '/model/%s'%MODEL_STEP3
...
def Generator_Adaptor(n_samples, noise=None):
    ...
    output = lib.ops.linear.Linear('%s_Adaptor_Generator.Input1'%ID_STUDENT, DIM, NOISE_LEN, noise)
    if MODE == 'wgan':
        output = lib.ops.batchnorm.Batchnorm('%s_Adaptor_Generator.BN1'%ID_STUDENT, [0], output)
    output = tf.nn.relu(output)

    output = lib.ops.linear.Linear('%s_Adaptor_Generator.Input2'%ID_STUDENT, NOISE_LEN, NOISE_LEN, output)
...
def Generator(n_samples, noise=None):
    ...
    output = lib.ops.linear.Linear('%s_Generator.Input'%ID_STUDENT, N_SAMPLES, 4*4*4*DIM, noise)
    if MODE == 'wgan':
        output = lib.ops.batchnorm.Batchnorm('%s_Generator.BN1'%ID_STUDENT, [0], output)
    output = tf.nn.relu(output)
    output = tf.reshape(output, [-1, 4*DIM, 4, 4])
...
fixed_noise_samples = Generator_student(N_SAMPLES, noise=fixed_noise)

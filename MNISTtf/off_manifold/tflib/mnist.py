    ...
    if selecting_label is None:
        ...
        if portions is None :
            for label in range(0, len(selecting_label)):
                if label==0:
                    images=images_aux[np.equal(targets_aux,selecting_label[label])][:int(limit/len(selecting_label))]
                    targets=targets_aux[np.equal(targets_aux,selecting_label[label])][:int(limit/len(selecting_label))]
                else:
            ...
            for label in range(0, len(selecting_label)):
                if label==0:
                    images=images_aux[np.equal(targets_aux,selecting_label[label])][:int(limit*portions[label])]
                    targets=targets_aux[np.equal(targets_aux,selecting_label[label])][:int(limit*portions[label])]
                else:
                    images=numpy.concatenate([images,images_aux[np.equal(targets_aux,selecting_label[label])][:int(limit*portions[label])]])
                    targets=numpy.concatenate([targets,targets_aux[np.equal(targets_aux,selecting_label[label])][:int(limit*portions[label])]])
...
def load(batch_size, test_batch_size, n_labelled=None, limit= None, selecting_label = None, bias = None, portions = None):
    ...
    #limit=len(test_data[1])
    #limit=len(train_data[1])
    #limit=len(dev_data[1])

    epoch_function_test=mnist_generator(test_data, test_batch_size, n_labelled, limit=limit, selecting_label = selecting_label, bias = bias, portions = portions)
    ...
    epoch_function_dev=mnist_generator(dev_data, test_batch_size, n_labelled, limit=limit, selecting_label = selecting_label, bias = bias, portions = portions)

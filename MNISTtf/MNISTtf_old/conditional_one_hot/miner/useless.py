...
FOLDER_NAME = 'useless_%s'%ID_TEACHER
...
fixed_noise = tf.random_normal([100, 128])
#fixed_noise_samples_student1 = Generator(128, noise= fixed_noise1 + TEACHER1_MEAN)
fake_data_teacher = teacher_model(noise = fixed_noise, fake_data_from_student = None, mode = MODE, id_= 'teacher2', dim = 64, batch_size = 100, critic_iters = CRITIC_ITERS, lambda_= LAMBDA, iters = ITERS, output_dim = OUTPUT_DIM, l2_lambda = L2_LAMBDA, adaptor_input_len = ADAPTOR_INPUT_LEN,real_data = real_data,n_classes=N_CLASSES, testing = True)
...
def generate_image_test(frame):
    ...
    lib.save_images.save_images(
        ...
        TEST_DIR+'pseudo_label_{}_samples_{}.png'.format(pseudo_label, frame)
    )

    print 'pseudo {}'.format(pseudo_label)
    return pseudo_label
...
with tf.Session() as session:
    ...
    pdb.set_trace()
    for i in xrange(1):
            generate_image(i, None)

    pseudo_n =[]
    for i in xrange(10000):
        label_p = generate_image_test(i)
        pseudo_n.append(label_p) 

    print 'target: {}, label form 0:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==0])/len(pseudo_n))
    print 'target: {}, label form 1:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==1])/len(pseudo_n))
    print 'target: {}, label form 2:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==2])/len(pseudo_n))
    print 'target: {}, label form 3:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==3])/len(pseudo_n))
    print 'target: {}, label form 4:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==4])/len(pseudo_n))
    print 'target: {}, label form 5:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==5])/len(pseudo_n))
    print 'target: {}, label form 6:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==6])/len(pseudo_n))
    print 'target: {}, label form 7:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==7])/len(pseudo_n))
    print 'target: {}, label form 8:{}%'.format(SELECTING_LABEL[0], 100.*len([1 for i in pseudo_n if i==8])/len(pseudo_n))
    for iteration in xrange(ITERS):
        ...
        if iteration > 0:
           ...
           # lib.plot.plot('%s/disc_fake_teacher2'%SAMPLES_DIR, _disc_fake_teacher2.mean())
            lib.plot.plot('%s/stastic_%d'%(SAMPLES_DIR, SELECTING_LABEL[1]), 1.0 * len(_pred_cls[_pred_cls == 8]) / BATCH_SIZE)

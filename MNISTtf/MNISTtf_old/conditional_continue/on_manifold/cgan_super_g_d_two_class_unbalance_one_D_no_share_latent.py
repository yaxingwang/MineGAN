...
SELECTING_LABEL = [5,-1 ] # digits are 1 and 4
NUM_PER1 = 1
NUM_PER2 =2999 
...
fake_data_set = teacher_model(noise = noise, fake_data_from_student = None, mode = MODE, id_= 'teacher2', dim = 64, batch_size = BATCH_SIZE, critic_iters = CRITIC_ITERS, lambda_= LAMBDA, iters = ITERS, output_dim = OUTPUT_DIM, l2_lambda = L2_LAMBDA, adaptor_input_len = ADAPTOR_INPUT_LEN, real_data = real_data,n_classes=N_CLASSES, soft=prob_soft)
...
# teacher1
#noise1 = tf.random_normal([BATCH_SIZE, 128])
fixed_noise,_, fixed_noise_soft  = Generator_Adaptor(100, noise = fixed_noise0) 
#fixed_noise_samples_student1 = Generator(128, noise= fixed_noise1 + TEACHER1_MEAN)
fake_data_teacher = teacher_model(noise = fixed_noise, fake_data_from_student = None, mode = MODE, id_= 'teacher2', dim = 64, batch_size = 100, critic_iters = CRITIC_ITERS, lambda_= LAMBDA, iters = ITERS, output_dim = OUTPUT_DIM, l2_lambda = L2_LAMBDA, adaptor_input_len = ADAPTOR_INPUT_LEN,real_data = real_data,n_classes=N_CLASSES, soft=fixed_noise_soft)

 
test_noise = tf.random_normal([1, ADAPTOR_INPUT_LEN])
test_noise,_, test_prob_soft  = Generator_Adaptor(1, noise = test_noise) 
#fixed_noise_samples_student1 = Generator(128, noise= fixed_noise1 + TEACHER1_MEAN)
test_data_teacher = teacher_model(noise = test_noise, fake_data_from_student = None, mode = MODE, id_= 'teacher2', dim = 64, batch_size = 1, critic_iters = CRITIC_ITERS, lambda_= LAMBDA, iters = ITERS, output_dim = OUTPUT_DIM, l2_lambda = L2_LAMBDA, adaptor_input_len = ADAPTOR_INPUT_LEN,real_data = real_data,n_classes=N_CLASSES, soft=test_prob_soft)
...
with tf.Session() as session:
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration > 0:
            ...
            _, _accuracy, _pred_cls = session.run([gen_train_op, accuracy, pred_cls], feed_dict={condector_label:np.reshape(_pseudo_label, (BATCH_SIZE, -1)), noise_data : read_noise, real_data: _data})
           ...
           # lib.plot.plot('%s/disc_fake_teacher2'%SAMPLES_DIR, _disc_fake_teacher2.mean())
            lib.plot.plot('%s/stastic_%d'%(SAMPLES_DIR, SELECTING_LABEL[0]), 1.0 * len(_pred_cls[_pred_cls == SELECTING_LABEL[0]]) / BATCH_SIZE)

...
RESULT_DIR = './result/%s/NOISE_LEN_%d_LIMIT_%d'%(ID, NOISE_LEN, LIMIT)
...
def Generator_Adaptor(n_samples, noise=None):
    ...
    output = lib.ops.linear.Linear('%s_Adaptor_Generator.Input1'%ID, 64, 128, noise)
    if MODE == 'wgan':
...
def Generator(n_samples, noise=None):
    ...
    output = lib.ops.linear.Linear('%s_Generator.Input'%ID, NOISE_LEN, 4*4*4*DIM, noise)
    if MODE == 'wgan':

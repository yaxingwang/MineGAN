...
def teacher_model(noise, fake_data_from_student, mode, id_, dim, batch_size, critic_iters, lambda_, iters, output_dim, l2_lambda, adaptor_input_len = 4, real_data =None):
    global MODE, ID, DIM, BATCH_SIZE, CRITIC_ITERS, LAMBDA, ITERS, OUTPUT_DIM, L2_LAMBDA,  ADAPTOR_INPUT_LEN, REAL_DATA
    MODE = mode 
    ID = id_
    DIM = dim
    ...
    if REAL_DATA is None:
        noise = Generator_Adaptor(BATCH_SIZE, noise = noise) 
        real_data = Generator(BATCH_SIZE, noise = noise)
    else:
        real_data = REAL_DATA

    fake_data = fake_data_from_student 
    ...
    return gen_cost, disc_cost, l2, real_data
    # For saving samples

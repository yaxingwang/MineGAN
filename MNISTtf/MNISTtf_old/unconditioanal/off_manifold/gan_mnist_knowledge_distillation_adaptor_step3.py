...
ADAPTOR_INPUT_LEN = 4
FOLDER_NAME = 'home1_step3/ada_input_len_%d'%ADAPTOR_INPUT_LEN
...
# model from step2
FOLDER_NAME_STEP2 = 'home1_step2/ada_input_len_%d'%ADAPTOR_INPUT_LEN
RESULT_DIR_STEP2 = './result/%s'%FOLDER_NAME_STEP2
MODEL_STEP2 = 'WGAN_GP.model-10000'
MODEL_DIR_STEP2 = RESULT_DIR_STEP2 + '/model/%s'%MODEL_STEP2
...
# teacher
noise = tf.random_normal([BATCH_SIZE, ADAPTOR_INPUT_LEN])
gen_cost, disc_cost, l2, _ = teacher_model(noise = noise, fake_data_from_student = fake_data, mode = MODE, id_= ID_TEACHER, dim = 64, batch_size = BATCH_SIZE, critic_iters = CRITIC_ITERS, lambda_= LAMBDA, iters = ITERS, output_dim = OUTPUT_DIM, l2_lambda = L2_LAMBDA, adaptor_input_len = ADAPTOR_INPUT_LEN)
...
#    disc_cost = tf.reduce_mean(disc_fake) - tf.reduce_mean(disc_real)
    ...
    gen_train_op = tf.train.RMSPropOptimizer(
        learning_rate=5e-5
    ).minimize(gen_cost + l2, var_list=gen_params)
...
# teacher
fixed_noise_teacher = tf.constant(np.random.normal(size=(128, ADAPTOR_INPUT_LEN)).astype('float32'))
_, _, _, fixed_noise_samples_teacher = teacher_model(noise = fixed_noise_teacher, fake_data_from_student = fake_data, mode = MODE, id_= ID_TEACHER, dim = 64, batch_size = BATCH_SIZE, critic_iters = CRITIC_ITERS, lambda_= LAMBDA, iters = ITERS, output_dim = OUTPUT_DIM, l2_lambda = L2_LAMBDA, adaptor_input_len = ADAPTOR_INPUT_LEN)

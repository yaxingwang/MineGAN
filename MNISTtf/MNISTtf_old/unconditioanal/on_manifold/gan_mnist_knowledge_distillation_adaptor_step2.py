...
SELECTING_LABEL = [0, 8] # digits are 1 and 4
LIMIT = 4000#I donot use 
ADAPTOR_INPUT_LEN = 4
#SELECTING_LABEL = None # digits are 1 and 4
#LIMIT = 50000

FOLDER_NAME = 'home1_step2/ada_input_len_%d'%ADAPTOR_INPUT_LEN
RESULT_DIR = './result/%s'%FOLDER_NAME
ADAPTOR = True
if ADAPTOR: 
    SAMPLES_DIR = RESULT_DIR + '/%d/samples/'%SELECTING_LABEL[0]
    LOGS_DIR = RESULT_DIR + '/%d/log/'%SELECTING_LABEL[0]
    MODEL_DIR = RESULT_DIR + '/%d/model/'%SELECTING_LABEL[0]
    TEST_DIR = RESULT_DIR + '/%d/test/'%SELECTING_LABEL[0]

    # model from step1
    FOLDER_NAME_STEP1 = 'home1_step1'
    RESULT_DIR_STEP1 = './result/%s'%FOLDER_NAME_STEP1
    MODEL_STEP1 = 'WGAN_GP.model-25000'
    MODEL_DIR_STEP1 = RESULT_DIR_STEP1 + '/model/%s'%MODEL_STEP1
else:
    SAMPLES_DIR = RESULT_DIR + '/samples_teacher/'
    LOGS_DIR = RESULT_DIR + '/log_teacher/'
    MODEL_DIR = RESULT_DIR + '/model_teacher/'
...
# For saving samples
fixed_noise_adaptor = tf.constant(np.random.normal(size=(128, ADAPTOR_INPUT_LEN)).astype('float32'))

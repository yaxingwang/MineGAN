...
import pickle
#note: get the pretrained model
#for every target number, we pretrain with source given the other 8
...
N_SAMPLES=20
FOLDER_NAME = 'home1_step1_nlen%s'%NOISE_LEN


if not np.array_equal(SELECTING_LABEL,[-1]):
...
if not np.array_equal(SELECTING_LABEL,[-1]):
	...
	train_gen, dev_gen, test_gen = lib.mnist.load(BATCH_SIZE, BATCH_SIZE, selecting_label=SELECTING_LABEL)
...
with tf.Session() as session:
    ...
    for iteration in range(ITERS):
        ...
        if iteration  == 0:
            generate_image_real(iteration, _data)

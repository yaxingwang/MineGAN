...
def get_fake_images_and_acts(inception, g_running, miner, miner_semantic, code_size, sample_num=5000, batch_size=16, device=None):
	dataset = TensorDataset(torch.randn(sample_num, code_size, device=device))
	#dataset = TensorDataset(torch.randn(sample_num, code_size))
	loader = DataLoader(dataset, batch_size=batch_size * torch.cuda.device_count())
	...
	for gen_in in loader:
		...
		with torch.no_grad():
                        if miner is not None:
                            fake_image = g_running(miner(gen_in), miner_semantic=miner_semantic)[0]# output is image and latent feature
                        else:
                            fake_image = g_running([gen_in])[0]# output is image and latent feature
                        out = inception(fake_image)
                        out = out[0].squeeze(-1).squeeze(-1)
